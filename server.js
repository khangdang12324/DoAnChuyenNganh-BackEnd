...
// Kết nối Docker (Bắt lỗi nếu không có)
let docker;
try {
    docker = new Docker();
} catch (e) {
    console.log("Docker không khả dụng khi khởi động.");
}
...
// ============================================================
// 2. MODELS (KHUÔN DỮ LIỆU)
// ============================================================
const UserSchema = new mongoose.Schema({
    username: { type: String, required: true, unique: true },
    password: { type: String, required: true }
});
// Kiểm tra model đã tồn tại chưa để tránh lỗi nạp lại
...
app.post('/register', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(400).json({ error: "Tên đăng nhập đã tồn tại hoặc lỗi server!" });
    }
});
...
app.post('/save-project', verifyToken, async (req, res) => {
    try {
        ...
        if (project) {
            ...
            project = new Project({ userId: req.userId, vfs });
            await project.save();
        }
...
const runWithDocker = async (language, code) => {
    ...
    // Nếu không có docker instance thì báo lỗi để chuyển sang exec
    ...
    const streamToString = (stream) => new Promise((resolve, reject) => {
        ...
        stream.on('end', () => {
             if (chunks.length > 0) resolve(Buffer.concat(chunks).toString('utf8').substring(8));
             else resolve("");
        });
...
const runCode = async (language, code) => {
    try {
        console.log("Thử chạy Docker...");
        return await runWithDocker(language, code);
    } catch (err) {
        console.log("Docker lỗi/không có. Chuyển sang Exec.");
        return await runWithExec(language, code);
    }
};
